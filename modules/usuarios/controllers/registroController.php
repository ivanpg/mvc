...
class registroController extends Controller {
    ...
    public function index() {
        ...
        if ($this->getInt('enviar') == 1) {
            ...
            //creamos mail activacion tenemos que estar en un servidor real
            //enviaremos el mail desde un servidor
            $mail->From = 'www.google.es';
            $mail->FromName = 'Tutorial MVC';
            $mail->Subject = 'Activacion de cuenta de usuario';
            $body = 'Hola <strong>' . $this->getSql('nombre') . '</strong>,' .
                    '<p>Se ha registrado en framework MVC. Para activar ' .
            ...
            //a donde denviar el mail
            $mail->AddAddress($this->getPostParam('email'));
            //$mail->Send();
            //para probar en locallhost teniendo activado el servidor de correo mercury
            mail($this->getPostParam('email'), 'Confirmación de Registro', $body, 'From: admin@sitename.com');
    ...
    public function activar($id, $codigo) {
        ...
        //lo cogemos otra vez asi podemos verificar si el estado cambió u ocurrió un error
        $row = $this->_registro->getUsuario(
                $this->filtrarInt($id), $this->filtrarInt($codigo)
        );
